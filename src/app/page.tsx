    ...
    <div className="min-h-screen bg-[#fcfaf7] font-serif selection:bg-amber-200 overflow-x-hidden">
      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? "bg-white/80 backdrop-blur-md py-4 shadow-sm" : "bg-transparent py-8"}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <img src="/icon.png" className="w-8 h-8 rounded-full grayscale opacity-50" alt="Logo" />
            <span className={`text-[10px] font-bold tracking-[0.4em] uppercase ${scrolled ? "text-black" : "text-gray-400"}`}>Consejero</span>
          </div>
          <div className="flex gap-8 items-center">
            <Link href="/admin" className="text-[10px] uppercase tracking-widest text-gray-400 hover:text-amber-600 transition-colors font-bold">Panel</Link>
            <Link href="/biblioteca" className="bg-black text-white px-8 py-2.5 rounded-full text-[10px] uppercase tracking-[0.2em] font-bold hover:bg-amber-700 transition-all shadow-xl shadow-black/10">Entrar</Link>
          </div>
        </div>
      </nav>
      ...
      <header className="relative h-screen flex items-center justify-center">
        ...
        <div className="relative z-20 text-center px-6 max-w-5xl animate-in fade-in zoom-in duration-1000">
          <img src="/icon.png" className="w-20 h-20 mx-auto mb-12 rounded-full shadow-2xl border border-amber-100 p-1 bg-white" alt="Logo Principal" />
          <h1 className="text-6xl md:text-9xl font-bold text-gray-900 tracking-tighter mb-8 leading-[0.85]">
            El Legado del <br/><span className="text-amber-600">Obrero</span>
          </h1>
          <p className="text-xl md:text-2xl text-gray-400 italic mb-14 leading-relaxed max-w-2xl mx-auto">
            "Instruye al niño en su camino, y aun cuando fuere viejo no se apartará de él."
          </p>
          <Link href="/biblioteca" className="group relative inline-flex items-center gap-4 px-14 py-6 bg-black text-white rounded-full font-bold text-xs uppercase tracking-[0.4em] overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
      ...
      <section className="max-w-6xl mx-auto px-6 py-40 border-t border-amber-100">
        <div className="grid md:grid-cols-2 gap-24 items-center">
          <div className="space-y-10">
            <span className="text-amber-600 font-bold text-[10px] uppercase tracking-[0.5em] block">Propósito y Visión</span>
            ...
            <p className="text-lg text-gray-500 leading-relaxed italic">
              Esta plataforma es la casa digital del ministerio de Jose Enrique Perez Leon. 
              Un espacio diseñado para que el obrero encuentre alimento espiritual de forma 
              accesible, ordenada y profunda.
            </p>
            <div className="flex items-center gap-6">
                <div className="h-px w-12 bg-amber-500"></div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Desde la Reina-Valera 1909</span>
            </div>
          </div>
          ...
          <div className="relative">
             <div className="aspect-[4/5] bg-neutral-900 rounded-[3rem] overflow-hidden shadow-2xl rotate-2 hover:rotate-0 transition-all duration-700">
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-12">
                   <img src="/icon.png" className="w-16 h-16 mb-8 grayscale invert opacity-20" alt="" />
                   <p className="text-white/40 text-xs tracking-[0.3em] uppercase mb-4">Colección Digital</p>
                   <h3 className="text-white text-2xl font-bold italic tracking-tight">"La verdad os hará libres"</h3>
                </div>
             </div>
